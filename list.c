...
{
assert(list!=NULL);
Node *new_node=_get_new_node(element);
...
uint32_t slist_compare(Slist *list1,Slist *list2)
{
    ...
    while(list1->length==list2->length && temp1->data==temp2->data && temp1->next!=NULL && temp2->next!=NULL)
    {
        temp1=temp1->next;
		temp2=temp2->next;
	}
		return (temp1->next==NULL && temp2->next==NULL);
	}
